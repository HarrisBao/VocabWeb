      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-30 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-6">
            <Link to="/student" className="font-black text-xl text-green-600 tracking-tight">
              IELTS Thanh Lê
            </Link>
            ...
            <nav className="hidden md:flex items-center gap-1">
              ...
              {SKILLS_LIST.map(skill => (
                <Link
                  ...
                  to={skill.route}
                  className={`px-3 py-2 rounded-lg text-sm font-bold transition-colors ${
                    location.pathname.startsWith(skill.route)
                      ? 'bg-green-50 text-green-700'
                      : 'text-gray-600 hover:bg-gray-100'
                  }`}
                >
          ...
          <div className="flex items-center gap-3">
            {/* Optional profile menu placeholder */}
        ...
        <div className="flex items-center justify-around h-16 px-2">
          <Link
            ...
            className={`flex flex-col items-center justify-center w-full h-full space-y-1 ${
              location.pathname === '/student' ? 'text-green-600' : 'text-gray-500 hover:text-gray-900'
            }`}
          >
          ...
          {SKILLS_LIST.map(skill => (
            <Link
              ...
              to={skill.route}
              className={`flex flex-col items-center justify-center w-full h-full space-y-1 ${
                location.pathname.startsWith(skill.route) ? 'text-green-600' : 'text-gray-500 hover:text-gray-900'
              }`}
            >
