...
export const ReviewSetWorkspacePage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const url = `/learn/review-set/${id}?classId=${classId}`;
        
        const [reviewRes, availRes] = await Promise.all([
          api.get<VocabularyReviewDto>(url),
          api.get<PracticeAvailabilityDto[]>(`/learn/review-set/${id}/practice/availability?classId=${classId}`)
        ]);
