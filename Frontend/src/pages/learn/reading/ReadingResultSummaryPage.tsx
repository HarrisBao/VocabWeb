...
export const ReadingResultSummaryPage: React.FC = () => {
  ...
  const handleRetake = async () => {
     try {
       ...
       navigate(/student/classes//reading//attempts/);
     } catch (e: any) {
    ...
    <div className="min-h-screen bg-gray-50 p-6 md:p-12 font-sans">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center space-x-4 mb-8">
          <Link to={/student/classes/} className="p-2 hover:bg-gray-200 rounded-full transition-colors">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
          ...
          <div className="flex flex-col gap-8">
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm flex flex-col gap-3">
              <Button onClick={() => navigate(/student/classes//reading//attempts//review)} className="w-full bg-brand-50 text-brand-700 hover:bg-brand-100 font-bold py-3 text-lg rounded-xl shadow-sm border border-brand-200">
                Xem lại bài làm
              </Button>
              ...
              <Button onClick={() => navigate(/student/classes/)} className="w-full bg-white text-gray-500 hover:text-gray-700 font-bold py-2 hover:bg-transparent">
                Về lớp
              </Button>
            </div>
          </div>
              ...
              {result.questionGroups.map((g: any, idx: number) => {
                ...
                return (
                  <div key={g.id} className="flex justify-between items-center p-4 bg-gray-50 rounded-xl border border-gray-100">
                    <div>
                      <h3 className="font-bold text-gray-800">{g.academicQuestionType?.replace(/_/g, ' ') || Phần }</h3>
